/* VIEW
   ...
   - 논리적 가상 테이블
    	-> 테이블 모양을 하고 있지만
    	실제로 값이 저장되어 있는 게 아니고, 가져와서 보여주는 것뿐임.
    ...
    ****VIEW 사용 시 주의사항 ****
    1) 가상의 테이블(실제 테이블 X) -> ALTER구문 사용 불가
    2) VIEW를 이용한 DML(INSERT/UPDATE/DELETE)가
       가능한 경우도 있지만
       많은 제약이 따르기 때문에 SELECT 용도로 사용하는 것을 권장한다.
    ...
    CREATE VIEW 뷰이름
    AS subquery; //테이블 복사와 비슷한데, 다른 점은 
    ...
    --      NOFORCE : 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성(기본값)
    	...
    	VIEW부터 만들어 놓고 TABLE을 나중에 만드는 겨
...
-- 사번,이름,부서명,직급명 조회 결과를 저장하는 VIEW를 생성해보자!
...
-- VIEW에 포함되지 않는 컬럼(DEPT_TITLE)에는
-- INSERT 시에 NULL이 삽입된다.

-- 만약 VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에
-- NOT NULL 제약조건이 설정되어 있는 경우
-- 정상적인 INSERT가 진행되지 않는다!
-- ORA-01400: NULL을 ("KH_LSY"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- VIEW의 본래 목적인 보여지는 것(조회)라는 용도에 맞게 사용하는 것이 좋다!
--> WITH READ ONLY 옵션으로 조회만 할 수 있게 강제할 수 있음.

CREATE OR REPLACE VIEW V_DCOPY2
AS SELECT DEPT_ID, LOCATION_ID FROM DEPARTMENT
WITH READ ONLY; -- 읽기 전용 VIEW를 생성(SELECT만 가능!)
